...
export default function LoginLogs() {
  ...
  const fetchStatistics = async () => {
    try {
      const response = await api.get('/login-logs/statistics');
  ...
  const filteredLogs = logs.filter(log => {
    ...
    if (!canViewAll && user && log.user?.id !== user.id) {
      return false;
    }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          {canViewAll ? 'Monitor all user login activity' : 'View your login history'}
        </p>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-blue-600 mt-1">{statistics?.today || 0}</p>
            </div>
            <div className="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-purple-600 mt-1">{statistics?.this_week || 0}</p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600 mt-1">{statistics?.unique_users || 0}</p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-orange-600 mt-1">{statistics?.unique_ips || 0}
              </p>
            </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        {filteredLogs.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-gray-500">No login logs found</p>
          </div>
        ) : (
                ...
                {filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-gray-50">
                    {canViewAll && (
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm">
                          <div className="font-medium text-gray-900">{log.user?.full_name || log.user?.email || 'Unknown'}</div>
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-500 max-w-md truncate" title={log.user_agent}>
                        {log.user_agent}
                      </div>
                    </td>
